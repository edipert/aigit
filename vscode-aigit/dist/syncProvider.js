...
class SyncProvider {
    constructor(extensionUri, workspaceRoot) {
        ...
        this._status = { tools: [], pendingSections: 0, conflictCount: 0 };
    }
    resolveWebviewView(webviewView, _context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(msg => {
            switch (msg.command) {
                case 'aigit.scan':
                    vscode.commands.executeCommand('aigit.scan');
                    break;
                case 'aigit.sync':
                    vscode.commands.executeCommand('aigit.sync');
                    break;
    ...
    runScan() {
        ...
        for (const def of defs) {
            ...
            for (const f of def.files) {
                if (fs.existsSync(path.join(this.workspaceRoot, f)))
                    found.push(f);
            }
            for (const d of def.dirs) {
                if (fs.existsSync(path.join(this.workspaceRoot, d)))
                    found.push(d + '/');
            }
            if (found.length > 0) {
    ...
    getHtmlContent() {
        ...
        const toolsHtml = tools.length > 0
            ? tools.map(t => {
                ...
                const fileLinks = t.files.map(f => `<span class="file-link" data-file="${f}">${f}</span>`).join(', ');
