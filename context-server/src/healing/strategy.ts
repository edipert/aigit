...
/** Structured healing plan that an AI agent can act on */
export interface HealingPlan {
    summary: string;
    failureCount: number;
    targets: HealingTarget[];
    contextBrief: string;
 ...
 */
export function buildHealingPlan(diagnosis: Diagnosis): HealingPlan {
    const targets: HealingTarget[] = diagnosis.failures.map(f => ({
        file: f.filePath,
        line: f.lineNumber,
        symbol: null,
    ...
    for (const target of targets) {
        ...
        if (match) target.symbol = match.name;
    }
    ...
    if (diagnosis.symbols.length > 0) {
        suggestedActions.push(
            `Inspect ${diagnosis.symbols.length} linked symbol(s): ${diagnosis.symbols.map(s => `@${s.name}`).join(', ')}`
        );
    }

    if (diagnosis.relatedDecisions.length > 0) {
        suggestedActions.push(
            `Review ${diagnosis.relatedDecisions.length} past decision(s) for conflict with current implementation`
        );
    }
 ...
 */
export function formatHealReport(plan: HealingPlan): string {
 ...
 */
export function formatHealPayload(plan: HealingPlan): string {
