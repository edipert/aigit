...
/** Enriched diagnosis with semantic context */
export interface Diagnosis {
    failures: TestFailure[];
    symbols: { name: string; type: string; file: string }[];
    relatedMemories: { id: string; content: string; type: string }[];
    relatedDecisions: { id: string; context: string; chosen: string }[];
    summary: string;
}
 ...
 */
export function parseTestOutput(raw: string): TestFailure[] {
 ...
 */
export function extractFailedSymbols(
    failures: TestFailure[],
    ...
    for (const failure of failures) {
        ...
        const fullPath = path.isAbsolute(failure.filePath)
            ? failure.filePath
 ...
 */
export async function queryHealingContext(
    symbols: { name: string; type: string; file: string }[],
    branch: string
): Promise<{
    memories: { id: string; content: string; type: string }[];
    decisions: { id: string; context: string; chosen: string }[];
}> {
...
export async function diagnoseTestFailure(
    ...
    if (symbols.length > 0) summary += ` Linked to ${symbols.length} symbol(s): ${symbolList}.`;
    if (contextCount > 0) summary += ` Found ${contextCount} related context entries.`;
    if (contextCount === 0) summary += ` No prior context found — this may be a new or untouched area.`;
